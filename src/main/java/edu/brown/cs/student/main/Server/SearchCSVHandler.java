...
public class SearchCSVHandler implements Route {
    ...
    public Object handle(Request request, Response response) throws Exception {
        ...
        if (this.loadCSVHandler.isLoaded()) {
            CSVParser<List<String>> csvParser = this.loadCSVHandler.getCSVParser(); // Add null checks / error handling
            ...
            if (columnIndexString != null) {
              ...
              try {
                ...
                return new SearchCSVFailureResponse("Index not entered as integer."); // specify failure (index not entered as number)
              }

              try {
                responseMap.put("Matching rows", search.searchCSV(toSearch, columnIndex));
              } catch (IndexOutOfBoundsException e) {
                return new SearchCSVFailureResponse("Column does not exist."); // specify failure (index doesn't exist / less than 0.)
              }
            ...
            if (headerName != null) {
              ...
              try {
                ...
                return new SearchCSVFailureResponse("Header does not exist."); // specify failure (header doesn't exist)
              }
            ...
            return new SearchCSVFailureResponse("CSV File not loaded.").serialize(); // specify failure (not loaded)
        }
    }
  ...
  public record SearchCSVSuccessResponse(String responseType, Map<String, Object> responseMap) {
    ...
    String serialize() { // error? check gearup code
      ...
      JsonAdapter<SearchCSVHandler.SearchCSVSuccessResponse> adapter = moshi.adapter(
          SearchCSVHandler.SearchCSVSuccessResponse.class);
  ...
  public record SearchCSVFailureResponse(String responseType, String errorDescription) {
    ...
    String serialize() { // error? check gearup code
      Moshi moshi = new Moshi.Builder().build();
      JsonAdapter<SearchCSVHandler.SearchCSVFailureResponse> adapter = moshi.adapter(
          SearchCSVHandler.SearchCSVFailureResponse.class);
      return adapter.toJson(this);
    }
  }
}
