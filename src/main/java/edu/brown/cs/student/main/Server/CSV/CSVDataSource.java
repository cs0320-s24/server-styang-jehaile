...
/**
 * This is our CSV data source class which is passed into our loadcsv, searchcsv, viewcsv handlers
 * to extract the data from the files. This class ensures that all the files must be loaded prior to
 * being searched or viewed by utilizing a boolean value.
 */
public class CSVDataSource {
  ...
  /**
   * LoadCSVData gets the absolute folder path of the file being used, concats that with the file
   * name passed in as a parameter, in the LoadCSVHandler, where this method is called, along with a
   * boolean indicating to the CSV parser whether here are headers present. This method reads the
   * data, creates a strategy object which handles strings, then passes these values into our CSV
   * parser which parses the data. Lastly the boolean is set to true indicating that the file was
  ...
  /**
   * This method handles the searching of the data, returning the search results as a list of lists
   * of strings. This method takes in the value to search for, the header name and the column index.
   * This method initializes an instance of the search class and passes in the parser. We make a
   * local variable to hold the integer value of the column index. We check whether there are column
   * indexes passed in through a try catch which catches illegal arguments. We then use the search
  ...
  /**
   * This method returns the contents of a csv file after it is parsed as a list of lists of strings
   * and adds the list of headers, if they are present in a data set to the start of what will
   * become the json. This method is called in the viewcsvhandler class.
   *
  ...
  /**
   * This method is called in our viewcsvhandler class and returns the boolean created in this class
   * which states the status of whether data is loaded.
   *
