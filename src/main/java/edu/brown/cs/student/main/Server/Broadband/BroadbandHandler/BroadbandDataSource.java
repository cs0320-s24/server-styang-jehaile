...
/**
 * Class that builds and requests queries from the census API, passing them to APIUtilities class to
 * deserialize, and returning the response as a BroadBand.
 */
public class BroadbandDataSource implements BroadbandDataSourceInterface {
  ...
  /**
   * This is the constructor for the broadbanddatasource class. Inside the constructor there is a
   * try catch which tries to call on the api to get the response and deserialize the state map. If
   * this is unsuccessful and any of the errors (IOException, URISyntax Exception, Interrupted
   * Exception are caught then the map is set to null.
   */
  public BroadbandDataSource() {
    try {
      HttpResponse<String> stateToCode =
          BroadbandAPIUtilities.getAPIResponse(
  ...
  /**
   * This method is a getter which takes in the state and county then has a return type of broadband
   * data. First the method null checks the state code map and throws an illegal state exception if
   * null. Then it ensures that the state is in the map. Next we make all the state names lower case
   * to ensure that capitalization does not intefere with finding the data. We then have a for loop
   * to build the list and extract the county code for the requested county. Lastly, this method
   * sends a request based on the state code and county code requested by the caller and returns the
   * deserialized data.
   *
   ...
   * @throws NoSuchElementException thrown if the data requested does not exist where it is
   *                                searched, initiating a failure response
   */
  public BroadbandData getBroadbandData(String state, String county)
    ...
    for (int i = 1; i < countyList.size(); i++) {
      if (countyList.get(i).get(0).toLowerCase().contains(county.toLowerCase() + " county,")) {
  ...
  /**
   * This method is used to send the request to the api to return a string of the results for the
   * state, county broadband data by using the HttpResponse class and the api utilities class which
   * gets the api response based on the state code and county code passed into this method.
   *
