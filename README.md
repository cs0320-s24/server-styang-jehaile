...
Link to repo: https://github.com/cs0320-s24/server-styang-jehaile.git
...
The server package is split into the CSV and Broadband packages which handle retrieving, converting, sending and other interaction with the csv files and broadband percentage census API respectively.
	...
	#CSV Package
		...
		##CSVDataSource class handles the interactions with the csv data files to load, view and 
...
Our test suite is split into four classes to check the unit test and the integration test of the broadband package and the csv package.
	...
	The ServerCSVIntegration tests confirm that the API server works as expected the JUnit integration tests. I set up the server thread and tear it down after the tests. 
### testLoadCSV
...
T his tests the contents of the view csv by checking that it contains the correct values by checking the string
...
That the user ensures they are putting in a valid county and state by displaying the associated json message
	...
	The ServerBroadbandUnitTest tests confirm that our program works even if a user wanted to use an API datasource locally that our program would still maintain functionality
...
Tests that the broadbandhandler displays a failure message
When requested without entering a county and a state
